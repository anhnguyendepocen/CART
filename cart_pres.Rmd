...
Predict black (0) or red (1) classification based on X1 and X2 values.
...
where $J$ is the number of regions, $R_{j}$ are the regions of predictors, and $\hat{y}_{R_{j}}$ is the mean repsonse for the _Jth_ region.
...
It displays the _primary_ split for each node as well as up to four "runner-up" splits (the splits that came closest to the chosen split in terms of increasing purity).
...
where $\alpha$ is the _complexity parameter_ ranging from 0 to $\infty$, $R(T)$ is the _risk_ of the tree (ie, the proportion misclassified), and $|T|$ is the number of terminal nodes (leaves) in a tree. Thus $R_{\alpha}(T)$ is the risk of a tree for a given $\alpha$.
...
The cross-validation results can be viewed with the `printcp` function. It prints 4 columns of numbers:
...
When it comes to classification problems, if the true decision boundary is linear, then a linear model will very likely outperform a tree-based model.
...
A linear model will outperform a tree model in this situation.
...
df <- df[order(df$x1),]
# boundary
df$b <- 2 + 0.8*seq(1,10,length=200)
...
Everitt, B. and Hothorn, T. (2006) _A Handbook of Statistical Analyses using R_. Chapman & Hall/CRC, Boca Raton. (Chapter 8)

Hastie, T. et al. (2009) _The Elements of Statistical Learning_. Springer, New York. (pages 308 - 310)

James, G. et al. (2013) _An Introduction to Statistical Learning_. Springer, New York. (Chapter 8)

Therneau, T. and Atkinson, E. (2014) _An Introduction to Recursive Partitioning Using the RPART Routines_. Included  `rpart` package
